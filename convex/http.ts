    ...
    switch (event.type) {
        ...
        case "user.updated":
            ...
            if (!event.data.public_metadata?.role) {
                console.warn(`⚠️ No role found in Clerk metadata for user ${event.data.id}`);
                return new Response("Missing role in metadata", { status: 400 });
            }
            switch (event.data.public_metadata?.role) {
                case "admin": {
                    const existingAdmin = await ctx.runQuery(internal.user.getAdmins, { clerkId: event.data.id });
                    if (!existingAdmin) {
                ...
                case "student": {
                    const existingStudent = await ctx.runQuery(internal.user.getStudent, { clerkId: event.data.id });
                    if (!existingStudent) {
                ...
                case "teacher": {
                    const existingTeacher = await ctx.runQuery(internal.user.getTeacher, { clerkId: event.data.id });
                    if (!existingTeacher) {
                ...
                case "parent": {
                    const existingParent = await ctx.runQuery(internal.user.getParent, { clerkId: event.data.id });
                    if (!existingParent) {
                ...
                default:
                    console.warn(`⚠️ Unrecognized role "${event.data.public_metadata?.role}" for user ${event.data.id}`);
            }
            break;
