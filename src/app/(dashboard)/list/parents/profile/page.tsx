...
export interface Student {
    id: string;
    lessons: any[];
    username: string; // Adjust type as per your lessons model
    surname: string;
    classes: any[];
    subjects: any[];
}
...
const SingleParentPage = () => {
        ...
        const fetchParent = async () => {
            if (id) {
                try {
                    ...
                    const studentResponse = await axios.get(`/api/students/getstudent/${parentData.studentId}`);
    ...
    const events = student ? convertToCalendarEvents(student.lessons) : [];
    console.log(events);
                        ...
                        <div className="flex flex-col justify-between gap-4 text-center md:text-left">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs font-medium">
                                ...
                                <div className="flex justify-center md:justify-start items-center gap-2">
                                    ...
                                    <span>
                                        {parent?.birthday
                                            ? moment(parent.birthday).format("DD MMMM YYYY")
                                            : ""}
                                    </span>
                                </div>
                                <div className="flex justify-center md:justify-start items-center gap-2 w-full">
                                    ...
                                    <span className="truncate w-full">{parent?.email}</span>
                                </div>
                                <div className="flex justify-center md:justify-start items-center gap-2 w-full">
                                    <Phone className="h-4 w-4 text-primary shrink-0" />
                                    <span className="truncate w-full">{parent?.phone}</span>
                                </div>
                            </div>
                        </div>
                        <Link
                            href={`/list/parents/manage?action=edit&id=${parent?.id}`}
